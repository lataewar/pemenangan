...
@section('content')
  ...
  <form action="{{ route('timses.update', ['timse' => $data->id]) }}" class="row" method="POST">
    @method('PUT')
    @csrf
    <div class="col-md-12">
      <div class="card card-custom card-stretch gutter-b">
        <div class="card-header">
          <h3 class="card-title">Ubah Tim Sukses</h3>
        </div>
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <x-validation.txt-stack type="email" name="email" placeholder="Email"
                value="{{ old('email') ?? $data->email }}" :messages="$errors->get('email')">Email (Email akan digunakan untuk login)
                <x-redstar /></x-validation.txt-stack>
              <x-validation.txt-stack type="text" name="no_tlp" placeholder="Nomor Telpon" value="{{ old('no_tlp') }}"
                value="{{ old('no_tlp') ?? $data->no_tlp }}" :messages="$errors->get('no_tlp')">Nomor Telpon
